...
public class Romes extends Number {
    ...
    private String convert_result_to_Romes(int n, int module) {
        module = n % 10;
        if (module != 0) {
            try {
                return convert_result_to_Romes(n - module, 0) + roman_letters_9[module - 1];
            } catch (ArrayIndexOutOfBoundsException e) {
                sign = "-";
                return convert_result_to_Romes(n - module, 0) + roman_letters_9[(module + 1) * -1];
            }
        }
    ...
    public void div() {
        try {
            ...
            System.out.print("Нельзя вводить римские и арабские цифры  одновременно. ");
            return;
        }
    ...
    private int convert_to_int(String romes_value){
        ...
        for (int i = 0; i < value_char.length; i++) {
              switch (value_char[i]) {
                  ...
                  default:
                      System.out.println("Проверьте правильность написания римских цифр:" + "\n" +
                              "I = 1" + "\n" +
                              "V = 5" + "\n" +
                              "X = 10");
                      break;
